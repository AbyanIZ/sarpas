...
<body
    ...
    <!-- Animated blobs -->
    <div
        class="absolute w-[200px] h-[200px] bg-[#5d6abf] rounded-full opacity-25 bottom-[-60px] left-[-60px] blur-md z-0"></div>
    <div
        class="absolute w-[300px] h-[300px] bg-[#2f3e8a] rounded-full opacity-25 bottom-[-100px] right-[-80px] blur-md z-0"></div>
    <div
        class="absolute w-[120px] h-[120px] bg-[#6a5acd] rounded-full opacity-25 top-[-40px] right-[-40px] blur-md z-0"></div>
    <div
        class="absolute w-[150px] h-[150px] bg-[#7d85e1] rounded-full opacity-20 top-[20%] left-[5%] blur-md z-0"></div>
    <div
        class="absolute w-[100px] h-[100px] bg-[#4d59c6] rounded-full opacity-25 top-[30%] right-[10%] blur-md z-0 animate-float"></div>
    <div
        class="absolute w-[180px] h-[180px] bg-[#7a6ee9] rounded-full opacity-20 top-[35%] left-[35%] blur-lg z-0 animate-float"></div>
    ...
    <nav
        ...
        <div class="flex items-center gap-4">
            ...
            <div
                class="w-9 h-9 flex items-center rounded-full overflow-hidden border-2 border-[#6aa6ff]/50">
    ...
    <div class="flex">
        <main class="flex-1 p-8 z-10">
            ...
            <div
                class="max-w-md mx-auto form-card bg-gradient-to-br from-[#1f1f1f] to-[#2a2a2a] p-8 rounded-xl shadow-lg border border-[#333]/50">
                <div class="flex items-center justify-center mb-6">
                    <div
                        class="w-16 h-16 rounded-full bg-[#6aa6ff]/10 flex items-center justify-center mr-4">
                        <i class="fas fa-box text-[#6aa6ff] text-2xl"></i>
                    </div>
                    <h2 class="text-xl font-bold">Form Tambah Barang</h2>
                </div>
                ...
                <form action="{{ route('barang.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                            class="w-full px-4 py-3 rounded-lg bg-[#2f2f2f] border border-[#333] input-field focus:outline-none focus:ring-2 focus:ring-[#6aa6ff] resize-none"></textarea>
                    </div>
                    ...
                    <div>
                        <label for="kategori_id" class="block text-sm font-medium mb-2 text-gray-300">
                            <i class="fas fa-list-alt mr-2 text-[#6aa6ff]"></i>Kategori
                        </label>
                    ...
                    <div>
                        <label for="gambar" class="block text-sm font-medium mb-2 text-gray-300">
                            <i class="fas fa-image mr-2 text-[#6aa6ff]"></i>Gambar Barang
                        </label>
                    ...
                    <p id="file-name" class="text-sm text-gray-400">Belum ada file yang dipilih</p>

                    <button type="submit"
                        class="w-full bg-gradient-to-r from-[#6aa6ff] to-[#4d73e6] hover:from-[#5a95e8] hover:to-[#3f63d4] text-white font-semibold py-3 px-4 rounded-lg transition-transform transform hover:scale-[1.02] flex items-center justify-center">
                ...
                <div class="mt-6 text-center">
                    <a href="{{ route('barang.index') }}"
                        class="text-[#6aa6ff] hover:underline flex items-center justify-center">
                ...
                <div class="mt-4 text-sm text-red-500">
                    ...
                    @enderror
                    @error('stock')
                    <p>Stok harus lebih besar dari 0.</p>
                    @enderror
                </div>
            </div>
        </main>
    ...
    <script>
        document.getElementById('gambar').addEventListener('change', function(event) {
            const fileName = event.target.files.length > 0 ? event.target.files[0].name : 'Belum ada file yang dipilih';
