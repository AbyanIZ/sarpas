...
<body class="min-h-screen font-sans bg-[radial-gradient(circle_at_top_left,_#202020,_#121212)] text-white relative overflow-x-hidden">
    ...
    <nav class="bg-[#1e1e1e]/95 backdrop-blur-md px-8 py-4 shadow-lg z-10 relative flex justify-between items-center border-b border-[#333]/50">
        <h1 class="text-2xl font-semibold bg-gradient-to-r from-[#6aa6ff] to-[#a162e8] bg-clip-text text-transparent flex items-center">
            <i class="fas fa-file-alt mr-2"></i>Laporan Peminjaman Barang
        </h1>
                ...
                <thead class="bg-gray-800/50">
                    <tr>
                        <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                            <i class="fas fa-user mr-1 text-[#6aa6ff]"></i> Nama Peminjam
                        </th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                        ...
                        <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                            <i class="fas fa-user-check mr-1 text-[#6aa6ff]"></i> Disetujui Oleh
                        </th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                ...
                <tbody class="bg-[#1f1f1f]/30 divide-y divide-gray-700/30">
                    @forelse ($peminjamans as $peminjaman)
                        <tr class="hover:bg-[#2a2a2a]/50 transition-colors text-center">
                            <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ $peminjaman->user->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ $peminjaman->barang->nama_barang }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-white font-semibold">{{ $peminjaman->jumlah }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-300">
                               @if ($peminjaman->approved_by)
                                    {{ $peminjaman->approvedBy->name }}
                                @else
                                    <span class="text-gray-500 italic">Belum disetujui</span>
                                @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-gray-400 font-mono">
                                {{ $peminjaman->created_at->translatedFormat('d F Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-400 font-mono">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-8 text-gray-400 italic">Data peminjaman tidak tersedia.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
