                    ...
                    <table class="w-full divide-y divide-gray-700/50">
                        <thead class="bg-gray-800/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                                    <i class="fas fa-image mr-1 text-[#6aa6ff]"></i> Foto
                                </th>
                                <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                                    <i class="fas fa-info-circle mr-1 text-[#6aa6ff]"></i> Keterangan
                                </th>
                                <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                                    <i class="fas fa-tag mr-1 text-[#6aa6ff]"></i> Status
                                </th>
                                <th class="px-6 py-3 text-xs font-medium text-gray-300 uppercase tracking-wider">
                                        ...
                                        @if ($pengembalian->image)
                                            <a href="{{ asset('storage/' . $pengembalian->image) }}" target="_blank"
                                                ...
                                                <img src="{{ asset('storage/' . $pengembalian->image) }}"
                                                    alt="Foto Pengembalian" class="w-12 h-12 object-cover rounded-lg group-hover:opacity-80 transition-opacity">
                                        @else
                                        ...
                                        @if ($pengembalian->status == 'pending')
                                            <div class="flex space-x-2 justify-center">
                                                <form action="{{ route('pengembalian.approve', $pengembalian->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="px-3 py-1 bg-green-600/90 hover:bg-green-700 text-white rounded-lg text-sm flex items-center">
                                                        <i class="fas fa-check mr-1"></i> Setuju
                                                    </button>
                                                </form>
                                                <form action="{{ route('pengembalian.reject', $pengembalian->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="px-3 py-1 bg-red-600/90 hover:bg-red-700 text-white rounded-lg text-sm flex items-center">
