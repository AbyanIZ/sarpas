...
<body class="min-h-screen font-sans bg-[radial-gradient(circle_at_top_left,_#202020,_#121212)] text-white relative overflow-hidden">
    ...
    <div class="flex">
        <main class="flex-1 p-10 z-10">
            ...
            <!-- Tombol Kembali ke Pendataan -->
            <a href="{{ route('pendataan') }}" class="text-blue-400 hover:underline mb-4 inline-block">← Kembali</a>
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold">Peminjaman Barang</h2></div>
            <!-- Tabel Peminjaman -->
            <div class="overflow-x-auto bg-[#1f1f1f]/80 p-6 rounded-2xl shadow-lg border border-[#333] mt-10">
                <table class="min-w-full divide-y divide-gray-700 text-white">
                    ...
                    <tbody class="bg-[#1f1f1f] divide-y divide-gray-700 text-white">
                        @foreach($peminjamans as $peminjaman)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $peminjaman->user->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $peminjaman->barang->nama }}</td>
                ...
                @if($peminjamans->isEmpty())
                    <p class="text-center py-6 text-gray-500">Belum ada peminjaman.</p>
                @endif
            </div>
        </main>
    </div>
