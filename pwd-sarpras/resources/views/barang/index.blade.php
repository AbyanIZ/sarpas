...
<body
    ...
    <nav class="bg-[#1e1e1e]/90 backdrop-blur-md px-10 py-4 shadow-lg z-10 relative flex justify-between items-center">
        <h1 class="text-3xl font-semibold">Pendataan Barang</h1>
        ...
        <main class="flex-1 p-10 z-10">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('barang.create') }}"
                    class="bg-blue-500 text-white px-4 py-2 rounded transition-transform hover:scale-105">+ Tambah
                    Barang</a>
            </div>
            ...
            <div class="space-y-4">
                @foreach ($barangs as $barang)
                    <div
                        class="bg-[#1f1f1f]/80 px-6 py-4 rounded-2xl shadow-lg border border-[#333] hover:bg-[#2a2a2a] transition-all flex items-center gap-6">
                        @if ($barang->gambar)
                            <img src="{{ asset('storage/' . $barang->gambar) }}"
                                class="w-24 h-24 object-cover rounded-md" />
                        @else
                            <div
                                class="w-24 h-24 bg-gray-700 rounded-md flex items-center justify-center text-sm text-gray-300">
                                No Image</div>
                        @endif

                        <div class="flex-1">
                            <h3 class="text-lg font-semibold mb-1">{{ $barang->nama_barang }}</h3>
                            <p class="text-sm text-gray-400"><span class="font-semibold text-white">Stock:</span>
                                {{ $barang->stock }}</p>
                            <p class="text-sm text-gray-400"><span class="font-semibold text-white">Deskripsi:</span>
                                {{ $barang->deskripsi }}</p>
                            <p class="text-sm text-gray-400"><span class="font-semibold text-white">Kategori:</span>
                                {{ $barang->kategori ? $barang->kategori->nama_kategori : 'Tidak Ada' }}</p>
                        </div>
